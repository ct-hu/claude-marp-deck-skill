...
> **build 時已自動執行（v1.6）**：`build_hybrid.py` 啟動會 `check_skill_freshness()`
> —— `git fetch` 比對 origin，本機落後就印 `[skill] ⚠️` 提示先 `git pull`（節流 10 分鐘、
> 離線自動略過），確保不會拿舊版規則做簡報。render 後跑 `lint_deck()` 印 `[lint]` 提醒。
> 所以下面的 Check 3 / 字面 markdown / 密度，看 build log 的 `[lint]` 即可；其餘 check 仍需人工 + 讀 PDF。

### 視覺硬規範（手刻 SVG / 排版守則）

反覆踩到的坑，固化成規則。多數由 `lint_deck()` 自動 WARN，但**修是 author 的事**：

1. **連接線 / 分支標籤不可裸壓在線上**（決策樹、箭頭圖最常見）——線會穿過字、糊掉。
   修法：標籤 `<text>` 前墊一塊與背景同色的 halo rect 擋掉線：
   `<rect x=".." y=".." width=".." height=".." fill="#FAF6EE" rx="3"/>`（深色框內改用該框底色）；
   或把標籤 offset 離開線段。**自動 lint：無**（幾何難測）→ 出圖後 Read PDF 確認。
2. **框內文字必須留 padding 完整落框**，不可頂邊 / 溢出。修法優先序：
   ① 加寬框（連動位移相鄰元素）② 換兩行 ③ 最後才 `textLength="(框寬-16)"`
   `lengthAdjust="spacingAndGlyphs"` 壓縮。**自動 lint：無**（需估字寬）→ Read PDF 確認。
3. **hand-SVG 最小字級 ≥ 18px**（≈ body 20px × 0.7 ÷ 0.78 scale；img-wide 縮更多要更高）。
   字小是因為塞太多元素 → 先減元素再放大，別硬壓字。**自動 lint：`lint_deck` WARN**。
4. **SVG `<text>` 不解析 markdown**：寫 `**粗體**` / `` `code` `` 會原樣顯示星號 / 反引號。
   粗體用 `<tspan font-weight="800">`、等寬用 `class="mono"`。**自動 lint：`lint_deck` WARN**。
   （副標 / callout 走 markdown → build_hybrid 的 `_inline_md_to_html` 已自動轉換，不受此限。）
5. **mermaid 序列圖字級天花板低**：mermaid-cli 11 把 actor / message 寫死 16px、忽略所有
   `sequence.*FontSize`。`mermaid-config.json` 已用 `themeCSS !important` 拉到 20/18，但受 box
   寬限制放大有限。**legibility 要緊的序列圖優先改手刻 SVG。**
6. **密度：一頁一主張（assertion-evidence）**——「一句結論 + 一個視覺證據」比塞滿 bullet 認知
   負荷低。一頁最多一個 key-concept callout；body 過長（`lint_deck` >360 字 / >19 行 WARN）就
   拆頁、或把次要內容壓進副標一行。
